...
def export_configuration():
    """Read and expose configuration to the package.

    Create a `block/_config.py` file with the package main metadata and options,
    so that they can be set as `block` attributes.
    """

    # Read `setup.cfg` configuration file.
    config = read_configuration("setup.cfg")
    metadata = config['metadata']

    # Check whether `block/_config.py` already exists or not.
    if not os.path.exists("block/_config.py"):
        # Create template for the configuration file.
        template = textwrap.dedent('''\
            """Expose configuration metadata to the package.

            This file is generated by `setup.py` in order to make package's
            configuration metadata easily avaiable from
            ...
            `AUTHOR` (str): Me!
            `EMAIL` (str): The reference email of the package.
            `DESCRIPTION` (str): A short description of the package.
            `LONG_DESCRIPTION` (str): A long description of the package, also including the license content.
            `URLS` (dict[str, str]): The reference urls of the package.
            `CLASSIFIERS` (list[str]): The classifiers of the package.
            `LICENSE` (str): The license name.

            Example
            -------
            ```python
            >>> from block import _config as cfg
            >>> cfg.NAME
            'block'
            >>> cfg.license
            'MIT'
            ...
            """
            __all__ = [
                'NAME', 'VERSION', 'AUTHOR', 'URLS',
                'EMAIL', 'DESCRIPTION', 'LONG_DESCRIPTION',
                'CLASSIFIERS', 'LICENSE'
                ]
            ...
            ''')
        args = [
            metadata[value].__repr__() for value in (
                'name', 'version', 'author', 'author_email',
                'description', 'long_description', 'project_urls',
                'classifiers', 'license'
                )
            ]
        content = template.format(*args)
        with open("block/_config.py", "w") as config_file:
...
def setup_package():
    ...
    # Expose package configuration to the package.
    if run_build:
