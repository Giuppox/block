...
# This is a bit hackish: Set a global variable so that the main block
# ``__init__.py`` can detect if it is being loaded by the setup routine, to
...
def parse_commands():
    ...
    Parse command-line-parsed arguments ``python setup.py <command>``.
    For passed info commands (e.g. ``--version``, ``--license``) don't run the
    build, the same for unsupported commands (e.g. ``--clean``, `'--bdist'`),
    that are actually supported by setuptools, but that are considered not
    usefull to the package or unsecure.
    For supported commands the package should be builded (e.g. ``build``,
    ``develop``).
    ...
    # Look for ``--force`` flag to know whether to run the setup in force mode
    # or not.
    force = '--force' in sys.argv
    # Remove ``--force`` flag from ``sys.argv``, so that setuptools doesn't
    # throws errors (unrecognised flag).
    if force:
    ...
    # Note that ``alias``, ``saveopts`` and ``setopt`` commands also seem to
    # work fine as they are, but are usually used together with one of the
    # commands below and not standalone. Hence they're not added to
    # ``build_commands``.
    # Also, ``install`` isn't added to ``supported_commands`` just because it
    # is handled later.
    ...
    # The following commands are supported, but there is the needing to show
    ...
    # only be executed when the user explicitly adds a ``--force`` command-line
    # argument.
    ...
    # If one command present in ``other_commands`` is passed then return
    # ``False`` (no need for module building).
    for command in other_commands:
        if command in args:
            return False

    # If the function hasn't recognized what ``setup.py`` command was given,
    # raise ``RuntimeError``.
